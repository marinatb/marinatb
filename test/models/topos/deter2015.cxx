...
TestbedTopology marina::deter2015()
{
  ...
  range(0,128)
    | for_each([&t,&mc_switches](int x){ 
         ...
         t.connect({c, c.ifx("ethX")}, mc_switches[x/32], 6_gbps); });
  ...
  range(0,110)
    | for_each([&t,&hp_switches](int x){
        ...
        t.connect({c, c.ifx("ethX")}, hp_switches[x/22], 20_gbps); });
  ...
  range(0,64)
    | for_each([&t,&pc2133_switches](int x){
        ...
        t.connect({c, c.ifx("ethX")}, pc2133_switches[x/27], 4_gbps); });
  ...
  range(0,18)
    | for_each([&t,&pc3060_switch](int x){
        ...
        t.connect({c, c.ifx("ethX")}, pc3060_switch, 5_gbps); });
